...
class Model_LinksType extends Model_Database {
	...
	public function install () {
		...
		if ($insert_data) {
			$sql	= 'INSERT INTO `'.$this->table.'` (`id`,`name`,`description`, `user_id`, `count`, `order`, `status`,`added`,`modified`)'
				    . ' VALUES '
				    . '(\'1\',\'Name-test-links-sample-one\',\'Description Test Links Sample One\', 1, 1, 1, \'publish\',NULL,NULL), '
				    . '(\'2\',\'Name-test-links-sample-two\',\'Description Test Links Sample Two\', 1, 1, 2, \'publish\',NULL,NULL);';
	...
	public function last_order($parent_id = '', $group = '') {
		...
		$sql = 'SELECT MAX(`order`) as `max_order` FROM `'.$this->table.'` '.$parent.';';
		$max_order = $this->manual_query($sql);
	...
	public function first_order($parent_id = '') {
		...
		// query to find default data 
		$sql = 'SELECT MIN(`order`) as `min_order` FROM `'.$this->table.'` '.$parent.';';
		$min_order = $this->manual_query($sql);
