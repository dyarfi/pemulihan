<?php defined('SYSPATH') or die('No direct script access.');
 ...
 class Kohana_Setting {
    protected $config = array (
            'view'      => 'setting/default',
            
            'model'     => array ('setting','global'),
            'perpage'   => 10,
        );   
    
    /**
     * Let's produce our setting, starting from here.
    **/
    
    public function get($config=array(),$paging=false){
        $config['limit'] = empty($config['limit']) ? $this->perpage : $config['limit'];
        $config['table'] = empty($config['table']) ? 'setting' : $config['table'];
        $tpl_var = array();
        if ($paging): 
            ...
            $tpl_var['perpage']     = $pagination->items_per_page;
        endif;
    ...
    **/
	...
	/**
	 * Enabled you for using config file and retrieves a setting config group from it. One config group can
	...
	public function config_group($group = 'default')
	{
		...
		$config_file = Kohana::$config->load('setting');
