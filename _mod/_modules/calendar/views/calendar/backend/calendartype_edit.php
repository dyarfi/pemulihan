...
<?php 
echo Form::open(URL::site('admin-cp/'.$class_name.'/edit/'.$links->id), array(
...
?>
	<div class="form_wrapper">
        <div class="form_row">
			...
			<?php echo Form::input('name',$fields['name'],array('id'=>'nametype', 'class'=>'required', 'rel'=>$class_name)); ?>
			<span class="red"></span>
            ...
            <fieldset style="clear:both;">
                ...
                <div class="form_row">
                    ...
                    <div class="form_fields">
                        <?php if (is_file(Lib::config($class_name.'.upload_path').$files[$row_name]->file_name) && in_array($files[$row_name]->file_type, $readable_mime)) : ?>
                        <div id="file_<?php echo $files[$row_name]->id; ?>">
                            <?php if (substr($files[$row_name]->file_type, 0, strlen('image/')) == 'image/') : ?>
                            <?php
                                $file_data	= pathinfo(url::base().Lib::config($class_name.'.upload_url').$files[$row_name]->file_name);
                                $thumb_ext	= isset($row_params['image_manipulation']['thumbnails'][0]) ? '_resize_'.$row_params['image_manipulation']['thumbnails'][0] : '';
                            ?>
                            <img src="<?php echo url::base().Lib::config($class_name.'.upload_url').$file_data['filename'].$thumb_ext.'.'.$file_data['extension']; ?>" alt="<?php echo url::base().Lib::config('links.upload_url').$files[$row_name]->file_name; ?>" />
                            ...
                            <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="320" height="20" id="FLVPlayer">
                                ...
                                <param name="FlashVars" value="file=<?php echo URL::site('admin-cp/'.$class_name.'/download/'.$files[$row_name]->file_name); ?>&amp;backColor=c2c2c2&amp;frontColor=666666&amp;showDownload=false&amp;repeatPlay=false&songVolume=100" />
                            ...
                            <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="320" height="240" id="FLVPlayer">
                                ...
                                <param name="scale" value="noscale" />
                                <param name="salign" value="lt" />
                                <param name="FlashVars" value="skinName=<?php echo url::base(); ?>flash/Corona_Skin_2&amp;streamName=<?php echo URL::site('admin-cp/'.$class_name.'/download/'.$files[$row_name]->file_name); ?>&amp;autoPlay=false&amp;autoRewind=false" />
                                <param name="swfversion" value="8,0,0,0" />
                                <!-- This param tag prompts users with Flash Player 6.0 r65 and higher to download the latest version of Flash Player. Delete it if you don�t want users to see the prompt. -->
                                <param name="expressinstall" value="Scripts/expressInstall.swf" />
                                <!-- Next object tag is for non-IE browsers. So hide it from IE using IECC. -->
                ...
                <div class="form_row">
                    ...
                    <div class="form_field"><a href="<?php echo URL::site('admin-cp/'.$class_name.'/download/'.$files[$row_name]->file_name); ?>"><img src="<?php echo IMG; ?>admin/disk.png" alt="<?php echo $files[$row_name]->file_name; ?>" /></a></div>
                </div>
