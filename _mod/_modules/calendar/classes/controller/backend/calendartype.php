...
class Controller_Backend_CalendarType extends Controller_Backend_BaseAdmin {
    ...
    public function action_index() {       
		...
		//$this->calendartype->find($where_cond);
		//exit();
		...
		if ($_POST) {
			...
			if ($this->id1 == 'select_action' && isset($post['check'])) {
				...
				foreach ($rows as $row) {
					$this->calendartype->id	= $row;

					if (!$this->calendartype->load())
						continue;

					$this->calendartype->status	= $post['select_action'];
					$this->calendartype->update();
				}
		...
		//$total_rows	= $this->calendartype->find_count($where_cond);
		
		$total_rows		= $this->calendartype->find_count($where_cond);
		$total_record 	= $total_rows;
		
		//$listings	= $this->calendartype->find($where_cond, array_merge($order_by, array('subject'=>'asc')), $per_page, $offset);
		
		$listings	= $this->calendartype->find($where_cond, $order_by, $per_page, $offset);
    ...
    public function action_add() {
			...
			if ($_POST) {
				...
				if ($post->check()) {
					...
					$id		= $this->calendartype->add($params);
...
//			$post->add_callbacks('name', array($this, '_unique_name'));
			...
			if ($post->check()) {
				...
				foreach ($params as $var => $val) {
					$this->calendartype->$var	= $val;
				}

				$this->calendartype->update();

				if (isset($this->_prefs['show_upload']) && $this->_prefs['show_upload'] && isset($this->_prefs['uploads'])) {
					$where_cond			= array('link_id'	=> $this->calendartype->id);
					...
					foreach ($this->_prefs['uploads'] as $row_name => $row_params) {
						...
						*/
						...
						if (!isset($files[$row_name])) {
							$params			= array('link_id'		=> $this->calendartype->id,
													'field_name'	=> $row_name,
				...
				$this->request->redirect('admin-cp/'.$this->_class_name.'/view/'.$this->calendartype->id);
				return;
			} else {
				$fields		= Arr::overwrite($fields, $post->as_array());
				$errors 	= Arr::overwrite($errors, $post->errors('validation'));
			...
			$fields	= array(
							...
							'description'		=> $this->calendartype->description,
							'status'			=> $this->calendartype->status);
    ...
    public function action_delete() {
		...
		// $this->calendartype->status	= 'deleted';
		
		// if($this->calendartype->update())
			// echo $this->id2;
		// else
			// echo 0;
	...
	public function action_check_name() {
		...
		$name_check =  $this->calendartype->load_by_name($_POST['name']);
	...
	public function action_change() {	
		if ($this->request->post('check') !='') {
			...
			foreach ($rows as $row) {
				...
				if (!$this->calendartype->load())
					continue;
				$this->calendartype->status	= $this->request->post('select_action');
				$this->calendartype->update();
			}
			$redirect_url	= (strstr($this->acl->previous_url,'admin-cp')) ? $this->acl->previous_url : 'admin-cp/'.$this->_class_name.'/index';
			$this->request->redirect($redirect_url);
    ...
    public function _unique_name (Validation $array, $field) {
		...
		$name = Model_CalendarType::instance()->find_count(array('name' => $array[$field]));
