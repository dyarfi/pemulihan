...
class Controller_ApiV2 extends Controller_Themes_DefaultRESTful {
	protected $id1;
	protected $id2;
	protected $id3;
	protected $id4;
	...
	public function action_articles () {
		...
		if ($this->id1 == 'all') {
			$articles				= Model_Article::instance()->find();
		} else {
			$articles				= Model_Article::instance()->load($this->id1);
		}
		//usleep(1200000);
	...
	public function action_medias () {
		...
		if ($this->id1 == 'all') {
			$medias	= Model_Media::instance()->find();
			
			$buffers = array();
			foreach ($medias as $media) {
				$media->files		 = Model_MediaFile::instance()->load_mp3_by_media_id($media->id);
				$buffers[$media->id] = $media; 
			}			
		} else {
			$medias				= Model_Media::instance()->load($this->id1);
		}		
		usleep(1200000);
	...
	public function action_members () {
		...
		if($this->id1 == 'register') {
			echo jason_encode($_POST);
			exit;
		}
		if ($this->id1 == 'all') {
			$medias	= Model_Member::instance()->find();
	...
	public function action_category () {
		...
		if ($this->id1 == 'calendar') {
			...
			$_calendar		= Model_Links::instance()->find(array('status'=>'publish'),array('order'=>'desc'));

			$buffers = array();
			foreach ($_calendar as $key) {
